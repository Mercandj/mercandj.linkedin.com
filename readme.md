# LinkedIn

-------------------------------------------------------------

## Links

- [PROFILE JONATHAN MERCANDALLI](https://www.linkedin.com/in/jonathan-mercandalli)

-------------------------------------------------------------

## Title

### English

```
Lead Android developer at MWM • FT120 🚀 • Android lover, my passion: make apps for everyone
```

### French

```
Lead Android developer chez MWM • FT120 🚀 • Passionné par Android, aime faire des apps utiles à tous
```

-------------------------------------------------------------

## Info

### English

```
👉 Android Lead at MWM, mobile apps software publisher.

➡️ What does MWM ?
🚀 MWM is a french scale-up of 150 people. Leader on Android and iOS with "creative" apps.
📈 More that 500 million downloads.
🎧 DJ and drawing apps the most downloaded in the Apple AppStore and the Google PlayStore.
💰 MWM is unique, the MWM's product is not one app or a chain of hits. The product of MWM is an ecosystem of apps.


➡️ Journey at MWM as Android Lead ?
👴 In 7 years at MWM, I saw the society grow up from 30 to 150 employees. I went from Dev to Android Lead, job that I'm doing since Jan 2021.
📱 I do management and individual contributions on projects to participate to app creation, new feature dev and tools for MWM's apps.
☯ 50% of management, 50% of individual contribution.


➡️ Hobbies & Side projects
🌍 Make and improve the protefolio on my home serveur. Ok course in Kotlin, of course in ktor.
🔗 https://mercandalli.com
♨️ Active contribution to open source projects on github.
🔗 https://github.com/Mercandj
📱 I create Android utility apps I love to use myself.


#android #mwm #playstore #google
```

### French

```
👉 Lead Android chez MWM, éditeur de logiciel d'apps sur mobile.

➡️ Que fait MWM ?
🚀 MWM est une scale-up française de 150 personnes qui est leader sur Android et iOS sur les apps "créatives".
📈 Pas mois de 500 millions de téléchargements à son actif.
🎧 Apps de DJ et de coloriage les plus téléchargées au monde.
💰 La particularité d'MWM, son produit n'est pas une app ou une série de hits, mais c'est un ecosystem d'apps.


➡️ Mon aventure chez MWM ?
👴 En 7 ans chez MWM, j'ai vu la société passer de 30 à 150 personnes. Après 5 ans comme dev Android, j'ai pris en charge le lead de l'équipe Android en Janvier 2021.
📱 Au quotidien, j'accompagne et participe à la création de nouvelles applications, de nouvelles fonctionnalités ainsi que l'amélioration de nos outils.
☯ 50% de management, 50% de contributions individuelles.


➡️ Hobbies & Side projects
🌍 Faire et améliorer le portfolio dont le serveur est chez moi. En Kotlin bien sur, en utilisant Ktor.
🔗 https://mercandalli.com
♨️ Des contributions à des projets open source sur github.
🔗 https://github.com/Mercandj
📱 J'adore créer des apps utiles au plus grand nombre


#android #mwm #playstore #google
```

-------------------------------------------------------------

## Experience

### MWM - Android Lead

#### English

```
Title: Android Lead
Date: January 2021, today
Where: Boulogne-Billancourt, Île-de-France, France
Description:

Few people know MWM. But MWM is a software publishier with more than 500 million downloads on Android and iOS. MWM is one of the 120 best french startup and raised 50,000,000€.

Great codebase, products and acquisition / monetization.

After 5 yeas as Android developer and contribute to launch a dozen apps, it is with pleasure that I continue the journey with the best team as Android Lead.

From DJ and the deep C++ audio loop, to the Compose Kotlin user interface of drawing apps, never 2 days are the same in my passion-job.
```

#### French

```
Title: Lead Android
Date: January 2021, today
Where: Boulogne-Billancourt, Île-de-France, France
Description:

Même si MWM n'est pas connu du grand public, elle compte 500 millions de téléchargements sur Android et iOS et rivalise avec les plus grands. Elle fait partie des 120 meilleures startups françaises.

Les forces d'MWM : sa codebase, la qualité de ses produits et la mise en place d'une monétisation / acquisition cohérente.

Après 5 ans comme développeur Android et avoir sorti d'une dizaine d'app, c'est avec plaisir que je continue l'aventure avec la meilleure équipe en tant que lead android.

Que ce soit en travaillant sur nos apps de musique, nos apps de coloriage ou nos libraries pour booster nos apps, jamais 2 journées se ressemblent dans mon métier-passion.
```

### MWM - Android Developer

#### English

```
Title: Android Developer
Date: October 2015, January 2021
Where: Boulogne-Billancourt, Île-de-France, France
Description:

I had the chance to work and to continue working with the best team on:

🎵 UI with custom view on vinyle turntable on edjing : DJ app n°1 on the stores
🎨 Drawing app creation with dynamic content on ColorPop : n°1 on the stores
📱 The Java to Kotlin transition, the Jetpack Compose adoption and to work on the pragmatic codebase evolution
🎧 Audio loop with JNI and C++
💰 And the monetization from PlayStore purchases or ads mediation
👉 In few words, make apps from A to Z.

MWM's strength lies in the fact that its product is not an app, no.
Its product is an ecosystem of several apps that complement each other.
```

#### French

```
Title: Développeur Android
Date: October 2015, January 2021
Where: Boulogne-Billancourt, Île-de-France, France
Description:

J'ai eu la chance de travailler et continuer à travailler avec la meilleure équipe sur :

🎵 De l'UI en custom view sur la platine vinyle sur edjing : app de DJ n°1 sur les stores
🎨 La création d'une app de dessin à contenu dynamic sur ColorPop : n°1 sur les stores
📱 Le passage en Kotlin, les débuts de Compose, une évolution de codebase pragmatique
🎧 Les boucles audio en JNI C++
💰 Et aussi sur la monetisation que ce soit via les purchases play store ou via les ads / ads mediation
👉 Bref, faire des apps de A à Z.

La force de MWM réside dans le fait que son produit n'est pas une app, non.
Son produit est un écosystème de plusieurs apps qui se complètent.
```

### Team Mercan - One-person business - Android developer

#### English

```
Title: Android Lead
Date: January 2013, today
Description:

I publish self made apps on the Google PlayStore under my personal account Team Mercan.
Build apps I want to use and share it to everyone is my goal.

Find all my apps on mercandalli.com and my open source projects on github.com/Mercandj
```

### Internship Assima

#### English

```
Title: Android Lead
Date: January 2013, today
Where: London
Description:

I worked for Assima in London as a C# developer on Vimago platform, a cloud based application.
Vimago brings tutorial to existing client apps (SAP...).

The goal was to modify the UI to have responsive mobile friendly interface. Technologies : a lot of CSS, JS, and few C#.
```

### Internship Datanoos

#### French

```
Title: Android Lead
Date: April 2014, Jully 2014
Where: Paris Area, France
Description:


Internship - Android developer in a 5 persons startupInternship - Android developer in a 5 persons startup
Datanoos · StageDatanoos · Stage
avr. 2014 - juil. 2014 · 4 moisavr. 2014 - juil. 2014 · 4 mois
Paris Area, FranceParis Area, France
"Aide à la Conception" et Réalisation de deux applications Android :
(1) Application mettant en contact clients/annonceurs. (disponible Google Play Store)
(2) Application de scan et extraction de contenu de tickets de caisse. (prototype)
```
